...
let raidenNewCache: unknown[] | null = null;
let raidenNewExpiry = 0;

async function fetchRaidenNewPairs(): Promise<unknown[]> {
  if (raidenNewCache && Date.now() < raidenNewExpiry) return raidenNewCache;
  try {
    // Fetch multiple pages and resolutions to get a broad set of RaidenX pairs
    const results = await Promise.allSettled([
      raidenFetch("/pairs/trending?page=1&limit=100&resolution=24h&network=sui"),
      raidenFetch("/pairs/trending?page=2&limit=100&resolution=24h&network=sui"),
      raidenFetch("/pairs/trending?page=1&limit=100&resolution=6h&network=sui"),
      raidenFetch("/pairs/trending?page=1&limit=100&resolution=1h&network=sui"),
    ]);
    ...
    for (const r of results) {
      ...
      for (const p of arr as Record<string, unknown>[]) {
        const id = ((p.poolId || p.pairId || "") as string).toLowerCase();
    ...
    // Filter: only pairs created in the last 7 days
    const sevenDaysAgo = Math.floor(Date.now() / 1000) - 7 * 24 * 3600;
    const newPairs = allPairs.filter((p) => {
      const ts = p.timestamp as number | string | null;
      if (!ts) return false;
      const tsNum = typeof ts === "number" ? ts : Math.floor(new Date(ts as string).getTime() / 1000);
      return tsNum > sevenDaysAgo;
    });
    raidenNewCache = newPairs;
    raidenNewExpiry = Date.now() + 30_000;
    return newPairs;
  } catch {
    return raidenNewCache || [];
  }
}
...
router.get("/new-pairs", async (req, res) => {
  try {
    ...
    // Fetch from both GeckoTerminal and RaidenX in parallel
    const [geckoResult, raidenRaw] = await Promise.allSettled([
      fetchGeckoNewPools(),
      fetchRaidenNewPairs(),
    ]);
    ...
    // Add GeckoTerminal new pairs
    if (geckoResult.status === "fulfilled") {
      const { pools, included } = geckoResult.value;
      for (const p of pools as Record<string, unknown>[]) {
        const formatted = formatGeckoPair(p, included as Record<string, unknown>[]);
        const addr = formatted.pairAddress.toLowerCase();
    ...
    // Add RaidenX new pairs (skip duplicates already in gecko results)
    if (raidenRaw.status === "fulfilled") {
      for (const p of raidenRaw.value as Record<string, unknown>[]) {
        const formatted = formatRaidenPair(p);
        const addr = formatted.pairAddress.toLowerCase();
        if (addr && !seenAddresses.has(addr)) {
          seenAddresses.add(addr);
          pairs.push(formatted);
